...
@section('title', 'Edit Kategori - Vyora Admin')

@section('content')

<div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-8">
  <div>
    <h2 class="text-2xl font-black text-slate-900 tracking-tight">Edit Kategori</h2>
    <p class="text-sm text-slate-500">Ubah kategori {{ $category->name }}</p>
  </div>
...
<div class="bg-white rounded-2xl border border-slate-200/80 shadow-xs p-8 max-w-xl">
  ...
  <form action="{{ route('admin.categories.update', $category->id) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
    ...
    <div>
      ...
      <input type="text" name="name" class="w-full px-4 py-2.5 rounded-xl border border-slate-300 focus:ring-2 focus:ring-indigo-500 text-sm font-medium transition" value="{{ old('name', $category->name) }}" required>
    </div>

    <div>
      <label class="block text-sm font-bold text-slate-900 mb-2">Gambar Kategori</label>
      @if($category->image)
        <div class="mb-3">
          <img src="{{ $category->image_url }}" alt="{{ $category->name }}" class="w-20 h-20 rounded-xl object-cover shadow-xs border border-slate-200">
        </div>
      @endif
